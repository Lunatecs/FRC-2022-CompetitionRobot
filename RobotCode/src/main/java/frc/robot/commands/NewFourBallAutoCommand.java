...
public class NewFourBallAutoCommand extends SequentialCommandGroup {
  /** Creates a new FourBallAutoCommand. */

  private final double kP = .055;

  public NewFourBallAutoCommand(IntakeSubsystem intake, DrivetrainSubsystem drive, TurretSubsystem turret, TowerSubsystem tower,
      ...
      addCommands(new InstantCommand(() -> intake.intakeOut()),
          ...
          new AutoStraightMoveCommand(drive, -1.6, angle, new PIDController(kP, 0, 0),.5),
         // new WaitCommand(0.5), // testing
          ...
          new InstantCommand(() -> drive.resetEncoders()),
          new AutoStraightMoveCommand(drive, -3.8, angle, new PIDController(kP, 0, 0)),
          new WaitCommand(.5),
          new InstantCommand(() -> drive.resetEncoders()),
          new AutoStraightMoveCommand(drive, 2.75, angle, new PIDController(kP, 0, 0),.5),
          new AutoAimCommand(turret, limelight),
          new InstantCommand(() -> stopper.stopperIn()),
          new WaitCommand(1.0),
          new InstantCommand(() -> {
